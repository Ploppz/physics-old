...
LineStrip Intersection::cast_shadow_on(Polygon* polygon, vec2 direction)
{
    const bool DEBUG = false;
    // Align transform makes `direction` toward +x
    mat2 align_transform = rotate_coor_system(direction);
    mat2 align_inverse = inverse(align_transform);
    ...
    vec2 this_min_y_vec(0, std::numeric_limits<float>::max()),
    ...
    /*** Find out if we have to swap start and end of LineStrip ***/
    ...
    if ( ! (using_this_min_y_vertex && !vertices[this_min_y_index].intersect) ) {
        ...
        vec2 next_edge_vec = (next_edge.end_tr() - next_edge.start_tr());
        vec2 prev_edge_vec = (prev_edge.end_tr() - prev_edge.start_tr());
